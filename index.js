...
const responseText = (hourlyPay, currency) => {
  console.log(
    `Your hourly pay in USD would change from $${hourlyPay.USD}/hr to $${hourlyPay[currency]}/hr if you were paid ${hourlyPay.salary} ${currency} instead of ${hourlyPay.salary} USD`
  );
};
...
rl.question("Enter a yearly salary in USD: ", function (salary) {
  ...
  rl.question(
    "Enter a three-letter ISO currency code (MXN, EUR, or CAD) to compare your hourly pay in a different currency: ",
    async function (currency) {
      ...
      try {
        ...
        if (!Object.keys(rates[0]).includes(cur) && cur !== "EUR") {
          console.log(
            "Sorry, but it doesn't seem that we have currency exchange info for that currency."
          );
          rl.close();
        }
